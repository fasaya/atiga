...
class Webadmin extends CI_Controller
{
    ...
    public function index()
    {
        ...
        // $query = $this->db->get_where('back_login', ['id_user' => $id_user])->row_array();
        $query = $this->db->query(' SELECT id_user
                                    FROM back_login
                                    WHERE id_user = "' . $id_user . '" ');
        $cek = $query->num_rows();

        if (($cek >= 1) && ($log_stat == TRUE) && ($ket == "backoffice")) {
    ...
    public function login()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $user = $this->db->get_where('back_login', ['username' => $username])->row_array();
